...
function OpenThenSetValue(Url,Width,Height,WindowObj,SetObj,pic){
	...
	return ReturnStr;
	 ...
	 window.open(Url,'newWin','modal=yes,width='+Width+',height='+Height+',top='+iTop+',left='+iLeft+',resizable=no,scrollbars=no');
	}
}
function setVal(v){
...
{  
    ...
    if (fileext=="gif" || fileext=="jpg" || fileext=="jpeg" || fileext=="bmp" || fileext=="png")
	  { if (EditorId==''){
		 insertHTMLToEditor('<img src="'+FileList+'" border="0" style="width:500px; margin-left:auto; margin-right:auto; display: block;" /><br/>');	
	  }else{
...
function OpenImgCutWindowWap(deloriginphoto,installdir,photourl){
	OpenImgCutWindows(deloriginphoto,installdir,photourl,$('#KS_photowap')[0]);
}
